...
async function robustFetchVerifiedSource(address, apiKey) {
  const url = `https://api.etherscan.io/api?module=contract&action=getsourcecode&address=${address}&chainId=8453&apikey=${apiKey}`;
    ...
    try {
      ...
      for (const tx of transactions) {
        if (!tx.to) {
          ...
          // Wait 10 seconds before attempting source fetch
